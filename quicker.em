...
macro InsertFileHeaderEN(hbuf, ln,szName,szContent)
{
    ...
    ln++
    InsBufLine(hbuf, ln,  " * \@brief Brief description of the implementation")
    ln++
    ...
    ln++
    InsBufLine(hbuf, ln, " * \@note created on @sz@-@szMonth@-@szDay@")
    ln++
    InsBufLine(hbuf, ln, " */")
    ln++
    InsBufLine(hbuf, ln, "")
...
/*****************************************************************************
 函 数 名  : InsertHeaderFileTemplate
 功能描述  : 插入英文头文件模板
 输入参数  : hbuf
             ln         行号, 从此行开始插入
 输出参数  : 无
 ...
 修改历史      :
  1.日    期   : 2026年1月26日
    ...
    修改内容   : 新生成函数
*****************************************************************************/
macro InsertHeaderFileTemplate(hbuf, ln)
{
	InsBufLine(hbuf, ln, "")
	ln++
    InsBufLine(hbuf, ln, "/*---------------------------------------------------------------------------*")
    ...
    InsBufLine(hbuf, ln, " *--------------------------------------------------------------------------*/")
	ln++
    InsBufLine(hbuf, ln, "#include <stdint.h>")
	ln++
    InsBufLine(hbuf, ln, "")
	ln++
    InsBufLine(hbuf, ln, "/*---------------------------------------------------------------------------*")
    ln++
    InsBufLine(hbuf, ln, " * Exported define")
    ln++
    InsBufLine(hbuf, ln, " *--------------------------------------------------------------------------*/")
	ln++
    InsBufLine(hbuf, ln, "")
    ...
    InsBufLine(hbuf, ln, "/*---------------------------------------------------------------------------*")
    ln++
    InsBufLine(hbuf, ln, " * Exported typedef")
    ln++
    InsBufLine(hbuf, ln, " *--------------------------------------------------------------------------*/")
	ln++
	InsBufLine(hbuf, ln, "")
...
/*****************************************************************************
 函 数 名  : CreateNewHeaderFile2
 功能描述  : 生成一个新的头文件，文件名可输入，新定制
 输出参数  : 无
 返 回 值  :
 调用函数  :
 被调函数  :
...
macro CreateNewHeaderFile2()
{
    ...
    szPreH = toupper (szFileName)
    szExt = toupper(szExt)
    szPreH = cat(szPreH,"_@szExt@")
    hOutbuf = NewBuf(sz) // create output buffer
    if (hOutbuf == 0)
        stop
    SetCurrentBuf(hOutbuf)
    InsertCPP(hOutbuf,0)
	InsertHeaderFileTemplate(hOutbuf, 4);
    HeadIfdefStr(szPreH)
}
...
{
    //看来InsBufLine是在之前插入，所以先插入后面的，再插入前面的
    InsBufLine(hbuf, ln, "")
